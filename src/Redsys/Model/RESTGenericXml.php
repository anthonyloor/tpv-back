...
use App\Redsys\Utils\RESTAnnotation;
use ReflectionClass;
			...
			public function toXml() {
				...
				if ($thisTag !== NULL) {
						...
						if ($xmlClass !== NULL) {
							...
							if ($obj !== NULL) {
								if (!$obj instanceof $xmlClass) {
									// Intentar convertir el string en un objeto de la clase esperada
									if (is_string($obj) && class_exists($xmlClass)) {
										$obj = new $xmlClass($obj);
									} else {
										continue;
									}
								}
			
								// ✔ Ahora podemos invocar el método sin errores
								$propClass = new ReflectionClass($xmlClass);
					...
					try {
						...
						dump("⚠ Excepción en parámetros XML: " . $e->getMessage());
					}
